...
function showHideMenu()
{
  ...
  document.getElementById('shuttleworth').style.display = 'none';
      ...
      for(gwid in data) {
        let geojsonLayerCircles = L.geoJSON(data[gwid], {
          pointToLayer: function (feature, latlng) {
            var circleLayer = L.circle(latlng, feature.properties.radius, {
              stroke: false,
              fillOpacity: 0.8,
              fillColor: "#0000FF",
...
function addMouseOverEvents(gwid) {
  try {
    loadedGateways[gwid].on('mouseover', function() {
      console.log("Mouseover "+gwid);
        loadedCircleLayers[gwid].setStyle({fillColor: '#FF0000'});
    });
    loadedGateways[gwid].on('mouseout', function() {
        loadedCircleLayers[gwid].setStyle({fillColor: '#0000FF'});
    });

    gatewayMarkers.on('clustermouseover', function(event) {
      console.log(event);
    })
    gatewayMarkers.on('clustermouseout', function(event) {

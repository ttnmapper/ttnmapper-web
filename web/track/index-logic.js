...
function getHistory(offset) {
      ...
      function(data) {
        ...
        data.points.forEach(function(obj) { 
          ...
          if(Number(obj.lat) < 1 && Number(obj.lat) > -1) {
            return;
          }
          if(Number(obj.lon) < 1 && Number(obj.lon) > -1) {
            return;
          }
          ...
          if(allPoints[obj.lat][obj.lon] === undefined) {
            ...
            point.latitude = Number(obj.lat);
            point.longitude = Number(obj.lon);
            point.altitude = Number(obj.alt)
            point.rssi = Number(obj.rssi);
            point.snr = Number(obj.snr);
            point.signal = signal;
            console.log(obj.fcount);
            point.fcount = Number(obj.fcount);
...
*/
      obj.gateways.forEach(function(gateway) {
        ...
        if(Number(obj.latitude) < 1 && Number(obj.latitude) > -1) {
          return;
        }
        if(Number(obj.longitude) < 1 && Number(obj.longitude) > -1) {
          return;
        }
        ...
        if(allPoints[obj.latitude][obj.longitude] === undefined) {
          ...
          point.latitude = Number(obj.latitude);
          point.longitude = Number(obj.longitude);
          point.altitude = Number(obj.altitude);
          point.rssi = Number(gateway.rssi);
          point.snr = Number(gateway.snr);
          point.signal = signal;
          point.fcount = Number(obj.counter);
...
function updateMaxDistanceLine(point) {
  ...
  if(point.longitude > gwLon) {
    maxDistanceLine.setText((point.distance/1000).toFixed(0)+"km", 
      {
        orientation: 'flip',
        center: true,
        offset: 24,
        attributes: {'font-weight': 'bold', 'font-size': '24', fill: "#d755b1"}
      }
    );
  } else {
    maxDistanceLine.setText((point.distance/1000).toFixed(0)+"km", 
...
function timeConverter(a){
  ...
  // } else {
  //     var offset = -1*a.getTimezoneOffset()/60;

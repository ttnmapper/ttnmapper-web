...
$gateway_id = $_REQUEST['gtwId'];
$gateway_id_sql = $gateway_id;

if(substr($gateway_id_sql, 0, 4) === "eui-") {
  $gateway_id_sql = substr($gateway_id_sql, 4);
  $gateway_id_sql = strtoupper($gateway_id_sql);
}
...
try 
{
  ...
  $sqlloc->bindParam(':gwaddr', $gateway_id_sql);
  $sqlloc->execute();
  ...
  $details['latitude'] = floatval($locresult['lat']);
  $details['longitude'] = floatval($locresult['lon']);
  $details['last_heard'] = floatval($locresult['last_heard']);
  $details['channels'] = floatval($locresult['channels']);
  $details['gtwId'] = $gateway_id;

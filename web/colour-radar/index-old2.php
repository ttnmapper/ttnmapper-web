...
try {
    ...
    if(!isset($_REQUEST['gwall']) or (isset($_REQUEST['gwall']) and $_REQUEST['gwall']!="none"))
    {
      ...
      foreach($stmt->fetchAll() as $k=>$v) {
        ...
        '<br />Lat, Lon: '.$locresult['lat'].', '.$locresult['lon'].
        ...
        '<ul>'.
          ...
          //     (isset($_REQUEST['hideothers'])?'&hideothers=on':'').
          //     '\">blocks</a><br>'.
          '<li><a href=\"//ttnmapper.org/colour-radar/?gateway[]='.urlencode($v['gwaddr']).
              '&type=radar'.(isset($_REQUEST['hideothers'])?'&hideothers=on':'').
              '\">radar</a><br>'.
          '<li><a href=\"//ttnmapper.org/alpha-shapes/?gateway[]='.urlencode($v['gwaddr']).

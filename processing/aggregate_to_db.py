...
def clear_gateway(db, gweui):
  cursor = db.cursor()
  cursor.execute('DELETE FROM 500udeg WHERE gwaddr=%s', [gweui])
  db.commit()
...
def main(argv):
  ...
  for gwrow in cur_gateways.fetchall():
    ...
    if(lat_max-1>lat_min or lon_max-2.5>lon_min):
      ...
      with open( os.environ['TTNMAPPER_HOME']+"/logfiles/aggregate_to_db_skipped.log", "a") as myfile:
    ...
    for i in np.nditer(lat_range):
      for j in np.nditer(lon_range):
        ...
        SELECT count(*) FROM temp_table
          ...
          AND time>%s
        """
        values = [block_start_lat, block_end_lat, block_start_lon, block_end_lon, gwid, moved]

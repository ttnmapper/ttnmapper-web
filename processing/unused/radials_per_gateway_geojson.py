...
def main(argv):
  ...
  for gwrow in cur_gateways.fetchall():
    ...
    cur_blocks.execute("SELECT lat, lon, rssiavg, rssimin, rssimax, snravg, snrmin, snrmax FROM 500udeg WHERE gwaddr=\""+gwid+"\"")
    ...
    for row in cur_blocks.fetchall():
      ...
      feature["style"]["z-index"] = str(rssi+130)
      feature["style"]["zindex"] = str(rssi+130)
      feature["style"]["zIndex"] = str(rssi+130)
      feature["properties"] = {}
    ...
    filename = output_folder+"/"+gwid+"/"+gwid+"_"+outputfile+".geojson"
